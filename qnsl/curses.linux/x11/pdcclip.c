...
/*static Atom _XA_UTF8_STRING(Display *dpy) {
...
#define _XA_UTF8_STRING 0
// #endif

static Boolean _convert_proc(Widget w, Atom *selection, Atom *target, Atom *type_return, XtPointer *value_return, unsigned long *length_return, int *format_return) {
	...
	if (*target == XA_TARGETS(XtDisplay(pdc_toplevel))) {
		...
		XmuConvertStandardSelection(pdc_toplevel, req->time, selection, target, type_return, &std_targets, &std_length, format_return);
		...
		*targetP++ = XA_STRING;
		*targetP++ = XA_UTF8_STRING((Display *)XtDisplay(pdc_toplevel));

		memmove((void *)targetP, (const void *)std_targets, sizeof(Atom) * std_length);
		...
		*format_return = 8;
		...
		return True;
	} else if (*target == XA_UTF8_STRING(XtDisplay(pdc_toplevel)) || *target == XA_STRING) {
		...
		*type_return = *target;
		...
		return XmuConvertStandardSelection(pdc_toplevel, CurrentTime, selection, target, type_return, (XPointer *)value_return, length_return, format_return);
}
...
static void _get_selection(Widget w, XtPointer data, Atom *selection, Atom *type, XtPointer value, unsigned long *length, int *format) {
...
int PDC_setclipboard(const char *contents, long length) {
	...
	if (XtOwnSelection(pdc_toplevel, XA_PRIMARY, CurrentTime, _convert_proc, _lose_ownership, NULL) == False) {
