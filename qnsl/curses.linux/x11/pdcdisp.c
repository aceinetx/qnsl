...
chtype acs_map[128] = {PDC_ACS(0),	PDC_ACS(1),	 PDC_ACS(2),	PDC_ACS(3),	 PDC_ACS(4),	PDC_ACS(5),	 PDC_ACS(6),	PDC_ACS(7),	 PDC_ACS(8),	PDC_ACS(9),	 PDC_ACS(10), PDC_ACS(11), PDC_ACS(12), PDC_ACS(13), PDC_ACS(14), PDC_ACS(15),
											 PDC_ACS(16), PDC_ACS(17), PDC_ACS(18), PDC_ACS(19), PDC_ACS(20), PDC_ACS(21), PDC_ACS(22), PDC_ACS(23), PDC_ACS(24), PDC_ACS(25), PDC_ACS(26), PDC_ACS(27), PDC_ACS(28), PDC_ACS(29), PDC_ACS(30), PDC_ACS(31),
											 ' ',					'!',				 '"',					'#',				 '$',					'%',				 '&',					'\'',				 '(',					')',				 '*',
											 ...
											 PDC_ACS(7),	PDC_ACS(8),	 '#',					0xa4,				 PDC_ACS(11), PDC_ACS(12), PDC_ACS(13), PDC_ACS(14), PDC_ACS(15), PDC_ACS(16), PDC_ACS(17), PDC_ACS(18), PDC_ACS(19), PDC_ACS(20), PDC_ACS(21), PDC_ACS(22),
											 PDC_ACS(23), PDC_ACS(24), PDC_ACS(25), PDC_ACS(26), PDC_ACS(27), PDC_ACS(28), PDC_ACS(29), PDC_ACS(30), 0xb7,
...
static void _display_cursor(int old_row, int old_x, int new_row, int new_x) {
	...
	if (old_row >= SP->lines || old_x >= COLS || new_row >= SP->lines || new_x >= COLS)
		return;
	...
	if (pdc_vertical_cursor) {
		...
		for (i = 1; i <= SP->visibility; i++)
			XDrawLine(XCURSESDISPLAY, XCURSESWIN, pdc_cursor_gc, xpos + i, ypos - pdc_app_data.normalFont->ascent, xpos + i, ypos - pdc_app_data.normalFont->ascent + pdc_fheight - 1);
	} else {
		...
		XFillRectangle(XCURSESDISPLAY, XCURSESWIN, pdc_cursor_gc, xpos, yp, pdc_fwidth, yh);
	}
...
void PDC_blink_text(XtPointer unused, XtIntervalId *id) {
	...
	if ((SP->termattrs & A_BLINK) || !pdc_blinked_off)
		XtAppAddTimeOut(pdc_app_context, pdc_app_data.textBlinkRate, PDC_blink_text, NULL);
}
...
int PDC_display_cursor(int oldrow, int oldcol, int newrow, int newcol, int visibility) {
...
#endif
{
	...
	if (pdc_blinked_off && (sysattrs & A_BLINK) && (attr & A_BLINK)) {
		...
		XFillRectangle(XCURSESDISPLAY, XCURSESWIN, gc, xpos, bounds.y, bounds.width, pdc_fheight);
	} else {
		/* Draw it */
		...
		if (attr & (A_LEFT | A_RIGHT | A_UNDERLINE)) {
			...
			if (attr & A_UNDERLINE)
				XDrawLine(XCURSESDISPLAY, XCURSESWIN, gc, xpos, ypos + 1, xpos + pdc_fwidth * len, ypos + 1);

			if (attr & A_LEFT)
				for (k = 0; k < len; k++) {
					int x = xpos + pdc_fwidth * k;
					XDrawLine(XCURSESDISPLAY, XCURSESWIN, gc, x, ypos - pdc_fascent, x, ypos + pdc_fdescent);
				}
