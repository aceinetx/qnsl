...
int wnoutrefresh(WINDOW *win) {
	...
	if (!win)
		return ERR;
	if (is_pad(win))
		return pnoutrefresh(win, win->_pad._pad_y, win->_pad._pad_x, win->_pad._pad_top, win->_pad._pad_left, win->_pad._pad_bottom, win->_pad._pad_right);
	...
	for (i = 0, j = begy; i < win->_maxy; i++, j++) {
		if (win->_firstch[i] != _NO_CHANGE) {
			...
			if (first <= last) {
				memcpy(dest + first, src + first, (last - first + 1) * sizeof(chtype));

				first += begx;
				last += begx;

				if (first < curscr->_firstch[j] || curscr->_firstch[j] == _NO_CHANGE)
...
int doupdate(void) {
	...
	for (y = 0; y < SP->lines; y++) {
		PDC_LOG(("doupdate() - Transforming line %d of %d: %s\n", y, SP->lines, (curscr->_firstch[y] != _NO_CHANGE) ? "Yes" : "No"));
		...
		if (clearall || curscr->_firstch[y] != _NO_CHANGE) {
			...
			while (first <= last) {
				...
				if (clearall)
					...
					while (first + len <= last && (src[first + len] != dest[first + len] || (len && first + len < last && src[first + len + 1] != dest[first + len + 1])))
						len++;
