...
bool wenclose(const WINDOW *win, int y, int x) {
	...
	return (win && y >= win->_begy && y < win->_begy + win->_maxy && x >= win->_begx && x < win->_begx + win->_maxx);
}
...
int ungetmouse(MEVENT *event) {
	...
	for (i = 0; i < 3; i++) {
		...
		if (bstate & ((BUTTON1_RELEASED | BUTTON1_PRESSED | BUTTON1_CLICKED | BUTTON1_DOUBLE_CLICKED) << shf)) {
