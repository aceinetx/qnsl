...
static unsigned long _process_key_event(XEvent *event) {
	...
	if (event->type == KeyRelease) {
		...
		if (SP->return_key_modifiers && IsModifierKey(keysym)) {
			switch (keysym) {
			case XK_Shift_L:
				key = KEY_SHIFT_L;
				break;
	...
	count = XwcLookupString(pdc_xic, &(event->xkey), buffer, buflen, &keysym, &status);
	...
	for (i = 0; key_table[i].keycode; i++) {
		if (key_table[i].keycode == keysym) {
			...
			if ((event->xkey.state & ShiftMask) || (key_table[i].numkeypad && (event->xkey.state & Mod2Mask))) {
...
static unsigned long _process_mouse_event(XEvent *event) {
	...
	case ButtonPress:
		...
		while (XtAppPending(pdc_app_context)) {
			...
			if (rel.type == ButtonRelease && rel.xbutton.button == button_no)
				...
				XSendEvent(XtDisplay(pdc_toplevel), RootWindowOfScreen(XtScreen(pdc_toplevel)), True, 0, &rel);
		}
	...
	case MotionNotify:
		MOUSE_LOG(("\nMotionNotify: y: %d x: %d Width: %d "
							 "Height: %d\n",
							 event->xbutton.y, event->xbutton.x, pdc_fwidth, pdc_fheight));
	...
	if (SP->mouse_status.changes & PDC_MOUSE_MOVED && (SP->mouse_status.button[button_no - 1] & BUTTON_ACTION_MASK) == BUTTON_PRESSED)
	...
	if (SP->mouse_status.x < 0 || SP->mouse_status.x >= SP->cols || SP->mouse_status.y < 0 || SP->mouse_status.y >= SP->lines)
		return -1;
...
static void _dummy_handler(Widget w, XtPointer client_data, XEvent *event, Boolean *unused) {
}
...
int PDC_kb_setup(void) {
	...
	if (Xim) {
		pdc_xic = XCreateIC(Xim, XNInputStyle, XIMPreeditNothing | XIMStatusNothing, XNClientWindow, XCURSESWIN, NULL);
	}
	...
	if (pdc_xic) {
		...
		im_event_mask |= ButtonPressMask | ButtonReleaseMask | ButtonMotionMask;

		XtAddEventHandler(pdc_drawing, im_event_mask, False, _dummy_handler, NULL);
		XSetICFocus(pdc_xic);
	} else {
