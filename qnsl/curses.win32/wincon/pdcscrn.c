...
static short realtocurs[16] = {COLOR_BLACK,			COLOR_BLUE,			COLOR_GREEN,		 COLOR_CYAN,		 COLOR_RED,			COLOR_MAGENTA,		 COLOR_YELLOW,		 COLOR_WHITE,
															 COLOR_BLACK + 8, COLOR_BLUE + 8, COLOR_GREEN + 8, COLOR_CYAN + 8, COLOR_RED + 8, COLOR_MAGENTA + 8, COLOR_YELLOW + 8, COLOR_WHITE + 8};

static short ansitocurs[16] = {COLOR_BLACK,			COLOR_RED,		 COLOR_GREEN,			COLOR_YELLOW,			COLOR_BLUE,			COLOR_MAGENTA,		 COLOR_CYAN,		 COLOR_WHITE,
															 COLOR_BLACK + 8, COLOR_RED + 8, COLOR_GREEN + 8, COLOR_YELLOW + 8, COLOR_BLUE + 8, COLOR_MAGENTA + 8, COLOR_CYAN + 8, COLOR_WHITE + 8};
...
typedef BOOL(WINAPI *SetConsoleScreenBufferInfoExFn)(HANDLE hConsoleOutput, PCONSOLE_SCREEN_BUFFER_INFOEX lpConsoleScreenBufferInfoEx);
typedef BOOL(WINAPI *GetConsoleScreenBufferInfoExFn)(HANDLE hConsoleOutput, PCONSOLE_SCREEN_BUFFER_INFOEX lpConsoleScreenBufferInfoEx);
...
static void _set_console_info(void) {
	...
	hSection = CreateFileMapping(INVALID_HANDLE_VALUE, 0, PAGE_READWRITE, 0, sizeof(console_info), 0);
...
static void _init_console_info(void) {
	...
	for (i = 0; i < 16; i++) {
		...
		RegQueryValueExA(reghnd, tname, NULL, NULL, (LPBYTE)(&(console_info.ColorTable[i])), &len);
	}

	/* Font info */

	RegQueryValueEx(reghnd, TEXT("FontSize"), NULL, NULL, (LPBYTE)(&console_info.FontSize), &len);
	RegQueryValueEx(reghnd, TEXT("FontFamily"), NULL, NULL, (LPBYTE)(&console_info.FontFamily), &len);
	RegQueryValueEx(reghnd, TEXT("FontWeight"), NULL, NULL, (LPBYTE)(&console_info.FontWeight), &len);

	len = sizeof(WCHAR) * 32;
	RegQueryValueExW(reghnd, L"FaceName", NULL, NULL, (LPBYTE)(console_info.FaceName), &len);
...
static COLORREF *_get_colors(void) {
	if (pGetConsoleScreenBufferInfoEx) {
		...
		return (status == ERR) ? NULL : (COLORREF *)(&(console_infoex.ColorTable));
	} else {
...
int PDC_scr_open(int argc, char **argv) {
	...
	std_con_out = pdc_con_out = GetStdHandle(STD_OUTPUT_HANDLE);
	...
	if ((str = getenv("PDC_RESTORE_SCREEN")) == NULL || *str != '0') {
		...
		pdc_con_out = CreateConsoleScreenBuffer(GENERIC_READ | GENERIC_WRITE, FILE_SHARE_READ | FILE_SHARE_WRITE, NULL, CONSOLE_TEXTMODE_BUFFER, NULL);
	...
	pGetConsoleScreenBufferInfoEx = (GetConsoleScreenBufferInfoExFn)GetProcAddress(h_kernel, "GetConsoleScreenBufferInfoEx");
	pSetConsoleScreenBufferInfoEx = (SetConsoleScreenBufferInfoExFn)GetProcAddress(h_kernel, "SetConsoleScreenBufferInfoEx");
...
int PDC_init_color(short color, short red, short green, short blue) {
	...
	if (color < 16 && !pdc_conemu) {
		...
		if (color_table) {
			color_table[pdc_curstoreal[color]] = RGB(DIVROUND(red * 255, 1000), DIVROUND(green * 255, 1000), DIVROUND(blue * 255, 1000));
