...
int putwin(WINDOW *win, FILE *filep) {
	...
	if (filep && fwrite(marker, strlen(marker), 1, filep) && fwrite(&version, 1, 1, filep) && fwrite(win, sizeof(WINDOW), 1, filep)) {
		int i;
...
WINDOW *getwin(FILE *filep) {
	...
	if (!filep || !fread(marker, 4, 1, filep) || strncmp(marker, "PDC", 3) || marker[3] != DUMPVER || !fread(win, sizeof(WINDOW), 1, filep)) {
		free(win);
		return (WINDOW *)NULL;
	}
