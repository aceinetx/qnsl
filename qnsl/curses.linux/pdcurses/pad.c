...
WINDOW *subpad(WINDOW *orig, int nlines, int ncols, int begy, int begx) {
	...
	if (begy < 0 || begx < 0 || (begy + nlines) > orig->_maxy || (begx + ncols) > orig->_maxx)
		return (WINDOW *)NULL;
...
int pnoutrefresh(WINDOW *w, int py, int px, int sy1, int sx1, int sy2, int sx2) {
	...
	if ((!w || !(w->_flags & (_PAD | _SUBPAD)) || (sy2 >= LINES) || (sx2 >= COLS)) || (sy2 < sy1) || (sx2 < sx1))
		return ERR;
	...
	while (sline <= sy2) {
		if (pline < w->_maxy) {
			memcpy(curscr->_y[sline] + sx1, w->_y[pline] + px, num_cols * sizeof(chtype));

			if ((curscr->_firstch[sline] == _NO_CHANGE) || (curscr->_firstch[sline] > sx1))
	...
	if (!w->_leaveit && w->_cury >= py && w->_curx >= px && w->_cury <= py + (sy2 - sy1) && w->_curx <= px + (sx2 - sx1)) {
...
int pechochar(WINDOW *pad, chtype ch) {
	...
	return prefresh(pad, pad->_pad._pad_y, pad->_pad._pad_x, pad->_pad._pad_top, pad->_pad._pad_left, pad->_pad._pad_bottom, pad->_pad._pad_right);
}
