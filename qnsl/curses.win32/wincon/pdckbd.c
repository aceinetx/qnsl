...
static int _get_key_count(void) {
	...
	if (KEV.bKeyDown) {
		...
		if ((vk == VK_MENU && KEV.uChar.UnicodeChar) || ((vk == VK_SHIFT || vk == VK_CONTROL || vk == VK_MENU) && vk == save_press)) {
			save_press = 0;
			num_keys = 1;
		}
	}
...
static int _process_key_event(void) {
	...
	if (key && (!(state & LEFT_ALT_PRESSED) || (state & RIGHT_ALT_PRESSED))) {
...
static int _process_mouse_event(void) {
	...
	if (MEV.dwEventFlags == 4) {
		SP->mouse_status.changes = (MEV.dwButtonState & 0xFF000000) ? PDC_MOUSE_WHEEL_DOWN : PDC_MOUSE_WHEEL_UP;
	...
	if (MEV.dwEventFlags == 8) {
		SP->mouse_status.changes = (MEV.dwButtonState & 0xFF000000) ? PDC_MOUSE_WHEEL_RIGHT : PDC_MOUSE_WHEEL_LEFT;
	...
	action = (MEV.dwEventFlags == 2) ? BUTTON_DOUBLE_CLICKED : ((MEV.dwEventFlags == 1) ? BUTTON_MOVED : BUTTON_PRESSED);

	for (i = 0; i < 3; i++)
		SP->mouse_status.button[i] = (MEV.dwButtonState & button_mask[i]) ? action : 0;
	...
	if (action == BUTTON_PRESSED && MEV.dwButtonState & 7 && SP->mouse_wait) {
		...
		if (event_count) {
			...
			for (i = 0; i < 3; i++) {
				if (SP->mouse_status.button[i] == BUTTON_PRESSED && !(ip.Event.MouseEvent.dwButtonState & button_mask[i])) {
	...
	for (i = 0; i < 3; i++) {
		...
		if (SP->mouse_status.button[i] == BUTTON_MOVED) {
			...
			if (SP->mouse_status.x == old_mouse_status.x && SP->mouse_status.y == old_mouse_status.y)
				return -1;
	...
	for (i = 0; i < 3; i++) {
		if (old_mouse_status.button[i] == BUTTON_CLICKED || old_mouse_status.button[i] == BUTTON_DOUBLE_CLICKED)
...
int PDC_get_key(void) {
	...
	if (!key_count) {
		...
		if (save_ip.EventType == MOUSE_EVENT || save_ip.EventType == WINDOW_BUFFER_SIZE_EVENT)
			key_count = 1;
...
int PDC_mouse_set(void) {
	...
	SetConsoleMode(pdc_con_in, SP->_trap_mbe ? (ENABLE_MOUSE_INPUT | 0x0088) : (pdc_quick_edit | 0x0088));
