...
#define RINT(name1, name2, value) {#name1, #name2, XtRInt, sizeof(int), APPDATAOFF(name1), XtRImmediate, (XtPointer)value}

#define RPIXEL(name1, name2, value) {#name1, #name2, XtRPixel, sizeof(Pixel), APPDATAOFF(name1), XtRString, (XtPointer) #value}

#define RCOLOR(name, value) RPIXEL(color##name, Color##name, value)

#define RSTRINGP(name1, name2, param) {#name1, #name2, XtRString, MAX_PATH, APPDATAOFF(name1), XtRString, (XtPointer)param}

#define RSTRING(name1, name2) RSTRINGP(name1, name2, "")

#define RFONT(name1, name2, value) {#name1, #name2, XtRFontStruct, sizeof(XFontStruct), APPDATAOFF(name1), XtRString, (XtPointer)value}

#define RCURSOR(name1, name2, value) {#name1, #name2, XtRCursor, sizeof(Cursor), APPDATAOFF(name1), XtRString, (XtPointer) #value}

static XtResource app_resources[] = {RINT(lines, Lines, -1),
																		 RINT(cols, Cols, -1),
...
static XrmOptionDescRec options[] = {COPT(lines),				COPT(cols),					 COPT(normalFont),				COPT(italicFont),			COPT(boldFont),					COPT(bitmap),						COPT(pixmap),
																		 COPT(pointer),			COPT(clickPeriod),	 COPT(doubleClickPeriod), COPT(scrollbarWidth), COPT(pointerForeColor), COPT(pointerBackColor), COPT(cursorBlinkRate),
																		 COPT(textCursor),	COPT(textBlinkRate),

																		 CCOLOR(Black),			CCOLOR(Red),				 CCOLOR(Green),						CCOLOR(Yellow),				CCOLOR(Blue),						CCOLOR(Magenta),				CCOLOR(Cyan),
																		 CCOLOR(White),

																		 CCOLOR(BoldBlack), CCOLOR(BoldRed),		 CCOLOR(BoldGreen),				CCOLOR(BoldYellow),		CCOLOR(BoldBlue),				CCOLOR(BoldMagenta),		CCOLOR(BoldCyan),
																		 CCOLOR(BoldWhite)};
...
#define RGB(R, G, B) (((unsigned long)(R) << 16) | ((unsigned long)(G) << 8) | ((unsigned long)(B)))
	...
	for (i = 16, r = 0; r < 6; r++)
		for (g = 0; g < 6; g++)
			for (b = 0; b < 6; b++)
				pdc_color[i++] = RGB(r ? r * 40 + 55 : 0, g ? g * 40 + 55 : 0, b ? b * 40 + 55 : 0);
	for (i = 0; i < 24; i++)
...
static void _get_icon(void) {
	...
	{
		XpmReadFileToPixmap(XtDisplay(pdc_toplevel), RootWindowOfScreen(XtScreen(pdc_toplevel)), (char *)pdc_app_data.pixmap, &icon_pixmap, &icon_pixmap_mask, NULL);
	} else if (pdc_app_data.bitmap && pdc_app_data.bitmap[0]) /* bitmap */
	{
		unsigned file_bitmap_width = 0, file_bitmap_height = 0;
		int x_hot = 0, y_hot = 0;

		rc = XReadBitmapFile(XtDisplay(pdc_toplevel), RootWindowOfScreen(XtScreen(pdc_toplevel)), (char *)pdc_app_data.bitmap, &file_bitmap_width, &file_bitmap_height, &icon_pixmap, &x_hot, &y_hot);
		...
		rc = XGetIconSizes(XtDisplay(pdc_toplevel), RootWindowOfScreen(XtScreen(pdc_toplevel)), &icon_size, &size_count);
		...
		XpmCreatePixmapFromData(XtDisplay(pdc_toplevel), RootWindowOfScreen(XtScreen(pdc_toplevel)), (max_width >= 64 && max_height >= 64) ? icon64 : icon32, &icon_pixmap, &icon_pixmap_mask, NULL);
	}
}
...
static void _handle_expose(Widget w, XtPointer client_data, XEvent *event, Boolean *unused) {
...
static void _handle_nonmaskable(Widget w, XtPointer client_data, XEvent *event, Boolean *unused) {
...
static void _handle_enter_leave(Widget w, XtPointer client_data, XEvent *event, Boolean *unused) {
...
static void _handle_structure_notify(Widget w, XtPointer client_data, XEvent *event, Boolean *unused) {
	...
	case ConfigureNotify:
			 ...
			 place_text and place_graphics in next Expose. */
		{
			...
			if (new_w != resize_window_width || new_h != resize_window_height) {
...
static void _pointer_setup(void) {
	...
	XRecolorCursor(XCURSESDISPLAY, pdc_app_data.pointer, &pointerforecolor, &pointerbackcolor);
}
...
int PDC_scr_open(void) {
	...
	pdc_toplevel = XtVaAppInitialize(&pdc_app_context, class_name, options, XtNumber(options), &argc, argv, NULL, NULL);

	XtVaGetApplicationResources(pdc_toplevel, &pdc_app_data, app_resources, XtNumber(app_resources), NULL);
	...
	italic_font_valid = pdc_fwidth == pdc_app_data.italicFont->max_bounds.width;

	bold_font_valid = pdc_fwidth == pdc_app_data.boldFont->max_bounds.width;
	...
	XtVaSetValues(pdc_toplevel, XtNminWidth, minwidth, XtNminHeight, minheight, XtNbaseWidth, 0, XtNbaseHeight, 0, XtNbackground, 0, XtNiconPixmap, icon_pixmap, XtNiconMask, icon_pixmap_mask, NULL);
	...
	if (!PDC_scrollbar_init(argv[0])) {
		...
		XtVaSetValues(pdc_toplevel, XtNwidth, pdc_wwidth, XtNheight, pdc_wheight, XtNwidthInc, pdc_fwidth, XtNheightInc, pdc_fheight, NULL);
	}
	...
	SP->termattrs = A_COLOR | A_ITALIC | A_UNDERLINE | A_LEFT | A_RIGHT | A_REVERSE;

	/* Add Event handlers to the drawing widget */

	XtAddEventHandler(pdc_drawing, ExposureMask, False, _handle_expose, NULL);
	XtAddEventHandler(pdc_drawing, StructureNotifyMask, False, _handle_structure_notify, NULL);
	XtAddEventHandler(pdc_drawing, EnterWindowMask | LeaveWindowMask, False, _handle_enter_leave, NULL);
	...
	if (pdc_app_data.cursorBlinkRate)
		XtAppAddTimeOut(pdc_app_context, pdc_app_data.cursorBlinkRate, PDC_blink_cursor, NULL);

	XtRealizeWidget(pdc_toplevel);

	/* Handle trapping of the WM_DELETE_WINDOW property */

	wm_atom[0] = XInternAtom(XtDisplay(pdc_toplevel), "WM_DELETE_WINDOW", False);

	/* Make sure we tell X that we'd like to take focus */
	wm_atom[1] = XInternAtom(XtDisplay(pdc_toplevel), "WM_TAKE_FOCUS", False);
	XSetWMProtocols(XtDisplay(pdc_toplevel), XtWindow(pdc_toplevel), wm_atom, 2);
	...
	_get_gc(&pdc_italic_gc, italic_font_valid ? pdc_app_data.italicFont : pdc_app_data.normalFont, COLOR_WHITE, COLOR_BLACK);

	_get_gc(&pdc_bold_gc, bold_font_valid ? pdc_app_data.boldFont : pdc_app_data.normalFont, COLOR_WHITE, COLOR_BLACK);

	_get_gc(&pdc_cursor_gc, pdc_app_data.normalFont, COLOR_WHITE, COLOR_BLACK);

	XSetLineAttributes(XCURSESDISPLAY, pdc_cursor_gc, 2, LineSolid, CapButt, JoinMiter);
...
int PDC_color_content(short color, short *red, short *green, short *blue) {
	XColor tmp;
	Colormap cmap = DefaultColormap(XCURSESDISPLAY, DefaultScreen(XCURSESDISPLAY));
