...
static int _copy_win(const WINDOW *src_w, WINDOW *dst_w, int src_tr, int src_tc, int src_br, int src_bc, int dst_tr, int dst_tc, bool _overlay) {
	...
	for (line = 0; line < ydiff; line++) {
		...
		for (col = 0; col < xdiff; col++) {
			if ((*w1ptr) != (*w2ptr) && !((*w1ptr & A_CHARTEXT) == ' ' && _overlay)) {
				*w2ptr = *w1ptr;
...
int _copy_overlap(const WINDOW *src_w, WINDOW *dst_w, bool overlay) {
	...
	return _copy_win(src_w, dst_w, src_start_y, src_start_x, src_start_y + ydiff, src_start_x + xdiff, dst_start_y, dst_start_x, overlay);
}
...
int copywin(const WINDOW *src_w, WINDOW *dst_w, int src_tr, int src_tc, int dst_tr, int dst_tc, int dst_br, int dst_bc, int _overlay) {
	...
	return _copy_win(src_w, dst_w, src_tr, src_tc, src_end_y, src_end_x, dst_tr, dst_tc, _overlay);
}
