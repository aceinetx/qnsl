...
static void _highlight(void) {
	...
	for (j = y_start; j <= y_end; j++)
		for (i = (j == y_start ? x_start : 0); i < (j == y_end ? x_end : COLS); i++)
...
static int _mouse_key(void) {
	...
	} else if ((!mbe || SP->mouse_status.button[1] & BUTTON_SHIFT) && changes & 2 && (SP->mouse_status.button[1] & BUTTON_ACTION_MASK) == BUTTON_CLICKED) {
		SP->key_code = FALSE;
		return _paste();
	}
	...
	for (i = 0; i < 3; i++) {
		if (changes & (1 << i)) {
			...
			if ((!(mbe & (BUTTON1_PRESSED << shf)) && (button == BUTTON_PRESSED))
					...
					|| (!(mbe & (BUTTON1_DOUBLE_CLICKED << shf)) && (button == BUTTON_DOUBLE_CLICKED))
	...
	if (changes & (PDC_MOUSE_WHEEL_UP | PDC_MOUSE_WHEEL_DOWN)) {
		if (!(mbe & MOUSE_WHEEL_SCROLL))
			SP->mouse_status.changes &= ~(PDC_MOUSE_WHEEL_UP | PDC_MOUSE_WHEEL_DOWN);
	}
...
int wgetch(WINDOW *win) {
	...
	if (!(win->_flags & _PAD) && ((!win->_leaveit && (win->_begx + win->_curx != SP->curscol || win->_begy + win->_cury != SP->cursrow)) || is_wintouched(win)))
		wrefresh(win);
	...
	for (;;) /* loop for any buffering */
	{
		...
		if (key == '\b') {
			if (SP->c_pindex > SP->c_gindex)
				SP->c_pindex--;
		} else if (SP->c_pindex < _INBUFSIZ - 2)
